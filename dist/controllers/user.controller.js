...
var getUserToken = function (_id) {
    var authenticatedUser = jsonwebtoken_1.default.sign({ _id: _id }, "express", {
        expiresIn: "7d",
    });
    ...
    return __generator(this, function (_b) {
        switch (_b.label) {
            ...
            case 1:
                ...
                if (existingUser) {
                    return [2 /*return*/, response.status(409).send("User already exists")];
                }
            ...
            case 3:
                user = _b.sent();
                return [2 /*return*/, response.status(201).send({ massage: "User created successfully" })];
            case 4:
                error_1 = _b.sent();
                console.log("error in create user", error_1);
                throw error_1;
...
var loginUser = function (request, response) { return __awaiter(void 0, void 0, void 0, function () {
    var _a, email, password, existingUser, isPasswordCorrect, token, error_2;
    return __generator(this, function (_b) {
        switch (_b.label) {
            case 0:
                _b.trys.push([0, 3, , 4]);
            ...
            case 1:
                existingUser = _b.sent();
                if (!existingUser) {
                    return [2 /*return*/, response.status(409).send({ massage: "User does not exist" })];
                }
            ...
            case 2:
                ...
                if (isPasswordCorrect) {
                    ...
                    return [2 /*return*/, response.send({ token: token, user: { name: existingUser.name, email: existingUser.email }, })];
                }
                else {
                    return [2 /*return*/, response.status(400).send({ massage: "Invalid credentials" })];
                }
                return [3 /*break*/, 4];
            case 3:
                error_2 = _b.sent();
                console.log('error in login user', error_2);
                throw (error_2);
